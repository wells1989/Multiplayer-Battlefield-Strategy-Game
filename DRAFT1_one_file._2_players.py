# UI changed and sounds added, to do: conditional mute or un_mute button based on sound_active True / False? also bonuses or grid size / player increase ...
...
player4_cell_color = (255, 92, 51)"""
    ...
    return WHITE, RED, GREEN, BLUE, BLACK, GRAY, LIGHT_GRAY, water_cell_color, desert_cell_color, mountains_cell_color, current_cell_color, player1_cell_color, player2_cell_color, player1, player2, selected_attribute, attribute_values, sound_active

def generate_round_variables(player1, player2):
    running = True
...
def start_game(player1, player2):
    player1.score, player2.score = (500, 500)
    title_font, font, grid, grid_window, screen, cell_size, cols, rows, grid_size = initialize_game(player1, player2)

    running, current_player, clicked_cell, info_message, game_active = generate_round_variables(player1, player2)
...
def draw_extra_buttons(current_player, title_font, player1, player2, grid_size, cell_size):
    ...
    if current_player == player1:
        pygame.draw.rect(screen, player1.color, (grid_size * cell_size + 5, cell_size * 2 + 70, cell_size * 2 - 10, 80))
        pygame.draw.rect(screen, LIGHT_GRAY, (grid_size * cell_size + 9, cell_size * 2 + 74, cell_size * 2 - 10 - 8, 80 - 8))

        pygame.draw.rect(screen, LIGHT_GRAY, (grid_size * cell_size + 5, cell_size * 2 + 170, cell_size * 2 - 10, 80))
    else:
        pygame.draw.rect(screen, LIGHT_GRAY, (grid_size * cell_size + 5, cell_size * 2 + 70, cell_size * 2 - 10, 80))

        pygame.draw.rect(screen, player2.color, (grid_size * cell_size + 5, cell_size * 2 + 170, cell_size * 2 - 10, 80))
        pygame.draw.rect(screen, LIGHT_GRAY, (grid_size * cell_size + 9, cell_size * 2 + 174, cell_size * 2 - 10 - 8, 80 - 8))
    ...
    for i, line in enumerate(player1_lines):
        ...
        screen.blit(line_render, (grid_size * cell_size + 50, cell_size * 2 + 80 + i * 20))  
    ...
    # (grid_size * cell_size + 5, cell_size * 2 + 70, cell_size * 2 - 10, 80))
    for i, line in enumerate(player2_lines):
        line_render = title_font.render(line, True, RED)
        screen.blit(line_render, (grid_size * cell_size + 50, cell_size * 2 + 180 + i * 20))             
    ...
    if info_message is not None:
        pygame.draw.rect(screen, current_cell_color, (0, grid_size * cell_size + 10, cell_size * 4, 37))
        if ":" in info_message:
        ...
        pygame.draw.rect(screen, BLACK, (grid_size * cell_size + 5, grid_size * cell_size - 40, cell_size - 10, rect_height))
        screen.blit(replay_message, (grid_size * cell_size + 25, grid_size * cell_size - 30))
        ...
        pygame.draw.rect(screen, BLACK, (cell_size * grid_size + cell_size + 5, grid_size * cell_size - 40, cell_size-10, rect_height))
        screen.blit(rules_message, (cell_size * grid_size + cell_size + 25, grid_size * cell_size - 30))


        # mute / unmute buttons
        max_height = 37
        pygame.draw.rect(screen, WHITE, (cell_size * grid_size + 60, grid_size * cell_size +5, cell_size * 2, 40))
        ...
        screen.blit(sound_icon, (cell_size * grid_size + 60, grid_size * cell_size + 5)) 



def draw_grid(rows, cols, grid, player1, player2, cell_size, title_font, icons):
    for row in range(rows):
        for col in range(cols):
            cell = grid[row][col]
...
def pygame_setup(screen, icons, current_player, title_font, player1, player2, grid_size, cell_size, rows, cols, grid):
    draw_attribute_table(screen, icons)
    draw_extra_buttons(current_player, title_font, player1, player2, grid_size, cell_size)
    draw_grid(rows, cols, grid, player1, player2, cell_size, title_font, icons)


## Main game loop
WHITE, RED, GREEN, BLUE, BLACK, GRAY, LIGHT_GRAY, water_cell_color, desert_cell_color, mountains_cell_color, current_cell_color, player1_cell_color, player2_cell_color, player1, player2, selected_attribute, attribute_values, sound_active = generate_game_variables()

title_font, font, grid, grid_window, screen, cell_size, cols, rows, grid_size, running, current_player, clicked_cell, info_message, game_active = start_game(player1, player2)
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            restart_button = pygame.Rect(grid_size * cell_size + 5, grid_size * cell_size - 40, cell_size - 10, 40)
            if restart_button.collidepoint(event.pos):
                title_font, font, grid, grid_window, screen, cell_size, cols, rows, grid_size, running, current_player, clicked_cell, info_message, game_active = start_game(player1, player2)
                continue
            rules_button = pygame.Rect(cell_size * grid_size + cell_size + 5, grid_size * cell_size - 40, cell_size-10, 40)
            if rules_button.collidepoint(event.pos):
                display_rules_popup()
                continue

            mute_button = pygame.Rect((cell_size * grid_size + 60, grid_size * cell_size + 5, 40, 40))
...
## PyGame visual updates
        pygame_setup(screen, icons, current_player, title_font, player1, player2, grid_size, cell_size, rows, cols, grid)   
